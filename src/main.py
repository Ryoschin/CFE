...
def main(log: logging.Logger, config: Config):
    ...
    while True:
        for account in config.accounts:
            if (
                account not in farmThreads
                and restarter.canRestart(account)
                and stats.getThreadStatus(account)
            ):
        ...
        for account in farmThreads:
            if not farmThreads[account].is_alive():
                ...
                stats.updateStatus(
                    account,
                    f"[red]ERROR - restart at {restarter.getNextStart(account).strftime('%H:%M:%S')}, failed logins: {stats.getFailedLogins(account)}",
                )
                log.warning(
